...
from keras.layers import (Input, Activation, BatchNormalization, Conv2D,
                          Dense, Dropout, Flatten, ZeroPadding2D,
...
LOGNAME = 'iceburger-fconv2d-train'
...
def conv2d_bn(x, filters, num_row, num_col, padding = 'valid',
    ...
    if K.image_data_format() == 'channels_first':
        ...
        bn_axis = 3
    x = ZeroPadding2D((1,1), name=padding_name)(x)
    x = Conv2D(
    ...
    x = MaxPooling2D((2,2),strides=(2,2), name=pool_name)(x)
...
def Conv2D_Model(include_top = True, weights=None, input_tensor = None,
    ...
    x = Lambda(lambda x: x, input_shape=input_shape)(image_input)
    ...
    x = conv2d_bn(x, NUM_CONV_FILTERS, CONV_FILTER_ROW, CONV_FILTER_COL, strides=(1,1),
                  name = "conv2d_bn4")
    x = ZeroPadding2D((1, 1), name="zeropadding")(x)
